...
struct DatumVrijeme {
	...
	char * GetDatumKaoNizKaraktera() {
		...
		itoa(*_sati, sati, 10);
		itoa(*_minuti, minuti, 10);
		
		int duzina = strlen(dan) + strlen(mjesec) + strlen(godina) + strlen(sati) + strlen(minuti) + 1 + 4;

		datum = new char[duzina];
		strcpy_s(datum, duzina, dan);
		strcat_s(datum, duzina, ".");
		strcat_s(datum, duzina, mjesec);
		strcat_s(datum, duzina, ".");
		strcat_s(datum, duzina, godina);
		strcat_s(datum, duzina, " ");
		strcat_s(datum, duzina, sati);
		strcat_s(datum, duzina, ":");
		strcat_s(datum, duzina, minuti);
		
		return datum;
	}
};

int sumaDana(DatumVrijeme d) {
	int suma = 0;
	suma = *d._godina * 365 + *d._mjesec * 30 + *d._dan+*d._sati*1440 + *d._minuti*60;
	return suma;
}

const DatumVrijeme rokZaPrijavu = { new int(5), new int(7), new int(2017), new int(12), new int(30) };

int sumaRokZaPrijavu = sumaDana(rokZaPrijavu);
...
struct Predmet {
	...
	void Unos(char const * naziv, int ocjena, DatumVrijeme datumUnosa) {
...
struct Kandidat {
	...
	void Unos(eNacinStudiranja nacinStudiranja, char const * imePrezime) {
	...
	//dodavanje predmeta za razrede koji nisu definisani enumeracijom
		...
		for (int i = 0; i < 4; i++)
		{
			if (_uspjeh[i] != NULL) {
				...
				{
					if (strcmp(_uspjeh[i]->_predmeti->_naziv, predmet._naziv) == 0) {
						return false;
					}
				}
			}
		...
		int sumaPredmet = sumaDana(*predmet._datumUnosa);
		if (sumaPredmet > sumaRokZaPrijavu) {
			return false;
		}

		int Razred = razred; // DRUGI = 2 
		Razred--; // DRUGI = 1 
		for (int i = 0; i < 4; i++)
		{
			if (_uspjeh[Razred] == NULL) {
